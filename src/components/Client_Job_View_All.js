...
const Client_Job_View_All = () => {
    ...
    const deleteJobPost = async id => {
        await axios.delete(`http://localhost:4000/job_posts/${id}`) 
        loadJobPosts()
    }
            ...
            <tbody>

                {
                    jobPosts.filter((job_post) => {
                        ...
                        if (searchValue == ""){
                            return job_post.job_status == "New Project"
                        }
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <Link className="btn btn-secondary" to={`/client_job_edit/${job_post.id}`} >Edit</Link> {" "}
                                <Link className="btn btn-secondary" to="/client_job_view_all" onClick={() => deleteJobPost(job_post.id)} >Delete</Link> {" "}
                                <StripeCheckout 
                                    stripeKey="pk_test_51InTAbGa01ffLY9g9jIxzK2A0ozZ1kUrtUtQvoO7lLh2md03LLyEFj0JSDrfUi5TqFrsL6BAnDBSk5VMPzvbxVKC00ywhtirpN"
                                    token={handleToken}
                                />
                            </td>
                        </tr>
...
export default Client_Job_View_All